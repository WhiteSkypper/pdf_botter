...
if (command) {
await AshiSingh.sendPresenceUpdate('composing', m.chat)
AshiSingh.sendReadReceipt(from, m.sender, [m.key.id])}
}
  //autoread gc only
  if (global.autoReadGc) {
  if (m.isGroup) { AshiSingh.sendReadReceipt(m.chat, m.sender, [m.key.id]) }
}
  //auto recording all
...
break
case 'todo':
await AshiSingh.sendMessage(m.chat, {document: './Pdf/todo.pdf' , mimetype: '', fileName:`uba danna pakak na.pdf` }, { quoted: m })     
break
case 'menu':
